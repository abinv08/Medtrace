...
const memoryPatients = new Map<string, any>();
  ...
  try {
    ...
    try {
      ...
      patients = Array.from(memoryPatients.values()).filter((p) => {
        ...
        if (
          caretakerId &&
          !p.assignedCaretakers?.some(
            (c: any) => (c._id?.toString() || c.toString()) === caretakerId.toString()
          )
        ) {
          match = false;
        }
...
export const createPatient = async (
  ...
  try {
    ...
    try {
      existingPatient = await Patient.findOne({ userId });
    } catch {
      for (const p of memoryPatients.values()) {
...
export const updatePatient = async (
  ...
  try {
    ...
    try {
      ...
      if (assignedCaretakers !== undefined) patient.assignedCaretakers = assignedCaretakers;
